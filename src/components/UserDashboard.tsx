...
interface UserDashboardProps {
  ...
  budgets: Budget[];
  goals: FinancialGoal[];
  onNavigate: (tab: 'dashboard' | 'transactions' | 'budgets' | 'goals' | 'advisor') => void;
...
export default function UserDashboard({ 
  ...
  // Sample static quick template shortcuts for instant registration
  const SUGGESTED_QUICK_ACTIONS = [
    { label: "Daily Espresso", amount: 4.50, category: "Food & Beverage", type: "expense" as const, desc: "Starbucks Espresso" },
    { label: "Uber Ride", amount: 22.80, category: "Transportation", type: "expense" as const, desc: "Rideshare home" },
    { label: "Weekly Groceries", amount: 95.00, category: "Food & Beverage", type: "expense" as const, desc: "Organic market grocery run" },
    { label: "Freelance Design Payout", amount: 450.00, category: "Freelance & Consulting", type: "income" as const, desc: "Interface design milestone asset wire" }
  ];
      ...
      <div className="relative overflow-hidden bg-emerald-950 text-white p-6 md:p-8 rounded-3xl shadow-sm border border-emerald-900 flex flex-col md:flex-row md:items-center md:justify-between gap-6">
        <div className="relative z-10 space-y-2">
          <div className="inline-flex items-center gap-1.5 px-3 py-1 bg-emerald-800/50 border border-emerald-700/65 rounded-full text-[11px] font-semibold text-emerald-300">
            <Sparkles className="w-3.5 h-3.5 text-amber-300" /> Executive Financial Console
          </div>
          ...
          <p className="text-sm text-emerald-200/80 max-w-xl font-sans font-light">
            Keep your absolute capital trajectory steady. Below is your isolated sandbox database showing local spend boundaries, targets, and recent records.
          </p>
        </div>
        ...
        >
          <Sparkles className="w-4 h-4 text-emerald-600 animate-pulse" /> Ask Gemini Advisor
        </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-white p-6 border border-gray-100 rounded-3xl shadow-xs hover:border-emerald-200 transition-all duration-250 flex flex-col justify-between">
          <div>
            <div className="flex items-center justify-between text-xs font-bold text-gray-400 tracking-wider uppercase font-sans">
              <span>Primary Wallet Net Liquidity</span>
              <div className="p-2 bg-emerald-50 text-emerald-700 rounded-xl">
                <DollarSign className="w-4 h-4" />
              </div>
            </div>
            <h3 className="text-3xl font-display font-bold text-gray-800 mt-4">
              ${metrics.netBalance.toLocaleString(undefined, { minimumFractionDigits: 2 })}
            </h3>
          </div>
          <p className={`text-xs mt-3 font-medium ${metrics.netBalance >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}>
            {metrics.netBalance >= 0 
              ? `+$${metrics.netBalance.toLocaleString()} liquidity surplus registered` 
              : `-$${Math.abs(metrics.netBalance).toLocaleString()} liquidity deficit registered`
            }
          </p>
        </div>
        ...
        <div className="bg-white p-6 border border-gray-100 rounded-3xl shadow-xs hover:border-emerald-200 transition-all duration-250 flex flex-col justify-between">
          ...
          <p className="text-xs text-gray-400 mt-3 font-sans">
            Category boundaries keeping tabs on passive leakage
          </p>
        </div>
        ...
        <div className="bg-white p-6 border border-gray-100 rounded-3xl shadow-xs hover:border-emerald-200 transition-all duration-250 flex flex-col justify-between">
          <div>
            <div className="flex items-center justify-between text-xs font-bold text-gray-400 tracking-wider uppercase font-sans">
              <span>Savings Conservation Rate</span>
          ...
          <div className="mt-3">
            ...
            <p className="text-[10px] text-gray-400 mt-1.5 font-sans">
              Target minimum threshold benchmark: 20.0%
            </p>
          </div>
        </div>
      </div>
          ...
          <div className="bg-white p-6 border border-gray-100 rounded-3xl shadow-xs">
            <div className="mb-4">
              <h3 className="text-sm font-bold text-gray-800 font-sans tracking-wide uppercase flex items-center gap-1.5">
                ...
                <span>Instant Shortcut templates</span>
              </h3>
              <p className="text-xs text-gray-400 font-sans mt-0.5">Click any recurring benchmark below to immediately record with today's date</p>
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {SUGGESTED_QUICK_ACTIONS.map((act, index) => (
                <button
                  key={index}
                  onClick={() => {
                    onAddTransaction({
                    ...
                    alert(`Recorded "${act.label}" for $${act.amount} successfully!`);
                  }}
                ...
                >
                  ...
                  <div className="flex items-center gap-1.5 shrink-0 ml-2">
                    <span className="text-xs font-mono font-bold text-gray-800">${act.amount.toFixed(2)}</span>
            ...
            <div className="flex items-center justify-between mb-4">
              <div>
                <h3 className="text-sm font-bold text-gray-800 font-display tracking-wide uppercase flex items-center gap-1.5">
                  ...
                  <span>Recent Transactions Flow</span>
                </h3>
                <p className="text-xs text-gray-400 font-sans mt-0.5">Quick oversight of the latest active registry items</p>
              </div>
              <button 
                onClick={() => onNavigate('transactions')}
                className="text-[11px] font-bold text-emerald-600 hover:text-emerald-700 transition flex items-center gap-0.5"
              >
                <span>Navigate Ledger</span> <ArrowRight className="w-3.5 h-3.5" />
              </button>
            </div>

            {recentTransactions.length === 0 ? (
              <div className="py-12 border border-dashed border-gray-100 rounded-2xl text-center text-gray-400 text-xs font-sans">
                <p className="font-semibold text-gray-600">Zero active items in sandbox ledger.</p>
                <p className="text-[10px] text-gray-400 max-w-xs mx-auto mt-1">Navigate to the Transactions Ledger tab, or use one of the custom instant shortcuts above key fields to get started.</p>
              </div>
            ) : (
        ...
        <div className="lg:col-span-4 space-y-6">
          ...
          <div className="bg-white p-6 border border-gray-100 rounded-3xl shadow-xs flex flex-col justify-between h-full max-h-[300px] lg:max-h-none overflow-y-auto">
            <div>
              <div className="flex items-center justify-between mb-4">
                <div>
                  <h3 className="text-sm font-bold text-gray-800 font-display tracking-wide uppercase flex items-center gap-1.5">
                    ...
                    <span>Capital Goals</span>
                  </h3>
                  <p className="text-xs text-gray-400 font-sans mt-0.5">Milestones tracker</p>
                </div>
                <button 
              ...
              {activeSavingMilestones.length === 0 ? (
                <div className="py-8 border border-dashed border-gray-100 rounded-2xl text-center text-gray-400 text-xs font-sans">
                  No active savings objectives registered.
                </div>
              ) : (
            ...
            <div className="pt-4 border-t border-gray-50 mt-4 text-[11px] text-gray-400 leading-relaxed font-sans">
              Allocate surplus capital efficiently into these target pots to protect emergency wealth.
            </div>
          </div>
          ...
          <div className="bg-white p-6 border border-gray-100 rounded-3xl shadow-xs">
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-xs text-gray-400 font-sans mt-0.5">Budget status limits tracking</p>
              </div>
              <button 
            ...
            {criticalBudgets.length === 0 ? (
              <div className="py-8 border border-dashed border-gray-100 rounded-2xl text-center text-gray-400 text-xs font-sans">
                No custom budget caps established.
              </div>
            ) : (
              <div className="space-y-4 font-sans">
                  ...
                  <div key={item.category} className="space-y-1.5">
                    <div className="flex items-center justify-between text-xs">
                      ...
                      <span className={`font-semibold shrink-0 ${item.isExceeded ? 'text-rose-600 font-bold' : 'text-gray-500'}`}>
                        ${item.spent.toFixed(0)} / ${item.limit.toFixed(0)} ({item.ratio.toFixed(0)}%)
                      </span>
                    </div>
                    <div className="w-full bg-gray-100 h-1.5 rounded-full overflow-hidden shrink-0">
                      <div 
                        className={`h-full rounded-full transition-all duration-300 ${item.ratio >= 100 ? 'bg-rose-500' : 'bg-emerald-500'}`}
                    ...
                    {item.isExceeded && (
                      <div className="inline-flex items-center gap-1 text-[10px] text-rose-600 font-semibold bg-rose-50 border border-rose-100 py-0.5 px-2 rounded-md">
                        <AlertTriangle className="w-3 h-3 text-rose-500" /> Boundary exceeded
                      </div>
                    )}
                  </div>
                ))}
